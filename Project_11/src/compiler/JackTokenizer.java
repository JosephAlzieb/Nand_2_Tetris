...
public class JackTokenizer {
  ...
  private int intVal;
  private static final ArrayList<String> libraries;
  private int pointer;
  ...
  public JackTokenizer(File file) {
    ...
    StringBuilder jackCode = new StringBuilder();
    while (scanner.hasNextLine()) {
      ...
      jackCode.append(strLine.trim());
    }
    // add tokens from the jackcode string into an arraylist of the tokens
    tokens = new ArrayList<>();
    while (jackCode.length() > 0) {
      while (jackCode.charAt(0) == ' ') {
        jackCode = new StringBuilder(jackCode.substring(1));
      }
      // keyword
      for (String word : keyWords) {
        if (jackCode.toString().startsWith(word)) {
          tokens.add(word);
          jackCode = new StringBuilder(jackCode.substring(word.length()));
        }
      ...
      // integer constant
        ...
        StringBuilder value = new StringBuilder(jackCode.substring(0, 1));
        jackCode = new StringBuilder(jackCode.substring(1));
        while (Character.isDigit(jackCode.charAt(0))) {
          value.append(jackCode.charAt(0));
          jackCode = new StringBuilder(jackCode.substring(1));
      ...
      // string constant
        ...
        while ((jackCode.charAt(0) != '\"')) {
          strString.append(jackCode.charAt(0));
          jackCode = new StringBuilder(jackCode.substring(1));

        }
        strString.append("\"");
        tokens.add(strString.toString());
        jackCode = new StringBuilder(jackCode.substring(1));

      }
      // identifier
      else if (Character.isLetter(jackCode.charAt(0)) || (jackCode.charAt(0) == '_')) {
        StringBuilder strIdentifier = new StringBuilder(jackCode.substring(0, 1));
        jackCode = new StringBuilder(jackCode.substring(1));
        while ((Character.isLetter(jackCode.charAt(0))) || (jackCode.charAt(0) == '_')) {
          strIdentifier.append(jackCode.substring(0, 1));
          jackCode = new StringBuilder(jackCode.substring(1));
        }

        tokens.add(strIdentifier.toString());
  ...
  private boolean hasComments(String strLine) {
    return strLine.contains("//") || strLine.contains("/*") || strLine.startsWith(" *");
